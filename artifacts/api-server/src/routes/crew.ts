...
const RegisterBody = z.object({
  ...
  username: z
    ...
    .max(50)
    .regex(/^[a-zA-Z0-9._-]+$/, "Username hanya boleh huruf, angka, titik, _ atau -"),
...
router.post("/auth/register", async (req, res): Promise<void> => {
  ...
  let [dept] = await db.select({ id: departmentsTable.id }).from(departmentsTable).limit(1);
  if (!dept) {
  ...
  const empCode = "KRW" + String(Date.now()).slice(-6);
  ...
  await db.insert(crewCredentialsTable).values({
    employeeId: emp.id,
    username,
    passwordHash,
  ...
  const sessionToken = createCrewSession(emp.id, username, false);
  res.status(201).json({
    sessionToken,
