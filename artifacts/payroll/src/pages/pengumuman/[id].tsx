  ...
  useEffect(() => {
    if (announcement) {
      form.reset({
        ...
        category: announcement.category as "info" | "warning" | "urgent",
        audience: announcement.audience,
        publishedAt: announcement.publishedAt.split("T")[0],
      });
    }
  }, [announcement, form]);

  const onSubmit = (data: FormValues) => {
    updateAnnouncement.mutate({ id: Number(id), data }, {
      onSuccess: () => {
      ...
      onError: () => {
        toast({ title: "Gagal", description: "Terjadi kesalahan saat memperbarui pengumuman", variant: "destructive" });
      },
    });
  };
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="flex justify-end gap-4">
                ...
                <Button type="submit" disabled={updateAnnouncement.isPending}>
                  {updateAnnouncement.isPending ? "Menyimpan..." : "Simpan Perubahan"}
                </Button>
              </div>
            </form>
          </Form>
