...
export default function EditSchedule() {
  ...
  const { data: schedule, isLoading } = useGetSchedule(Number(id));
  ...
  const onSubmit = (data: FormValues) => {
    updateSchedule.mutate({ id: Number(id), data }, {
      onSuccess: () => {
      ...
      onError: () => {
        toast({ title: "Gagal", description: "Terjadi kesalahan saat memperbarui jadwal", variant: "destructive" });
      },
    });
  };

  if (isLoading) return <div className="text-center py-12 text-muted-foreground">Memuat data...</div>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-bold tracking-tight">Edit Jadwal Kerja</h1>
      </div>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="flex justify-end gap-4">
                ...
                <Button type="submit" disabled={updateSchedule.isPending}>
                  {updateSchedule.isPending ? "Menyimpan..." : "Simpan Perubahan"}
                </Button>
              </div>
            </form>
          </Form>
